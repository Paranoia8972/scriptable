...
// Fetch IP address and create widget
const ipAddress = await fetchIPAddress();
const widget = createWidget(ipAddress);
...
function createWidget(data) {
  ...
  const lastLoginLine = stack.addText(
    `Last login: ${timeFormatter.string(new Date())} on ttys001`
  );
  ...
  const nextPersonalCalendarEventLine = stack.addText(
  ...
  const nextWorkCalendarEventLine = stack.addText(
  ...
  const weatherLine = stack.addText(
    `${data.weather.icon} | ${data.weather.temperature}° (${data.weather.high}°-${data.weather.low}°), ${data.weather.description}, feels like ${data.weather.feelsLike}°`
  );
  ...
  // Line 6 - IP
  const ipLine = stack.addText(`🌐 | ${data.ip}`);
  ipLine.textColor = new Color(COLORS.ip);
  ipLine.font = new Font(FONT_NAME, FONT_SIZE);

  // Line 7 - Various Device Stats
  const deviceStatsLine = stack.addText(
    `📊 | ⚡︎ ${data.device.battery}%, ☀ ${data.device.brightness}%`
  );
...
async function fetchWeather() {
  ...
  const url =
    ...
    "&lon=" +
    location.longitude +
    "&exclude=minutely,hourly,alerts&units=" +
    TEMP_UNIT +
    "&lang=en&appid=" +
    WEATHER_API_KEY;
...
async function fetchNextCalendarEvent(calendarName) {
  ...
  const upcomingEvents = events
    ...
    .filter(
      (e) =>
        e.attendees &&
        e.attendees.some((a) => a.isCurrentUser && a.status === "accepted")
    );
